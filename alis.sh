...
LUKS_KEY_FILE="/crypto_keyfile.bin"
CRYPTTAB=""
LVM_VOLUME_GROUP="CryptVG"
...
function configuration() {
  ...
  if [ "$FULL_DISK_ENCRYPTION" == "true" ]; then
      dd if=/dev/random of="/mnt$LUKS_KEY_FILE" bs=512 count=4 iflag=fullblock status=progress
      chmod 600 "/mnt$LUKS_KEY_FILE"
      echo -n "$LUKS_PASSWORD" | cryptsetup luksAddKey "$PARTITION_ROOT" "/mnt$LUKS_KEY_FILE"

      if [ "$BOOTLOADER" == "grub" ]; then
    CRYPTTAB="rootfs UUID=$UUID_ROOT $LUKS_KEY_FILE luks"
      fi

      if [ "$BOOTLOADER" == "systemd" ]; then
    CRYPTTAB="cryptroot UUID=$UUID_ROOT $LUKS_KEY_FILE none"
      fi
...
function mkinitcpio_configuration() {
  ...
  if [ "$FULL_DISK_ENCRYPTION" == "true" ]; then
      arch-chroot /mnt sed -i "s/^FILES=(.*)$/FILES=(\\$LUKS_KEY_FILE)/" /etc/mkinitcpio.conf
  fi
...
function vagrant() {
  ...
  create_user "vagrant" "vagrant" "wheel"
